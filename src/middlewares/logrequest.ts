...
async function LogRequest(req: Request, res: Response, next: NextFunction): Promise<void> {
  if (!req.url.includes('ping')) {
    ...
    if (workspaceId) logger.info(`HTTP [${req.method}] ${req.url} | ${workspaceId} | client IP ${clientIp}`);
